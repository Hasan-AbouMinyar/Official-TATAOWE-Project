<template>
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="mb-6">
        ...
        <div class="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p class="text-gray-600">Update your volunteering opportunity</p>
          </div>
        </div>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="bg-white rounded-2xl shadow-sm p-8">
      ...
      <!-- Form -->
      <form v-else @submit.prevent="handleSubmit" class="bg-white rounded-2xl shadow-sm p-6 space-y-6">
        <!-- Event Photo Upload -->
        ...
        <div v-if="success" class="bg-green-50 border border-green-200 rounded-lg p-4">
          <div class="flex">
            ...
            <p class="text-green-700">Event updated successfully!</p>
          </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pt-4 border-t">
          ...
          <button
            type="submit"
            :disabled="submitting"
            class="px-6 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
          >
...
async function loadEvent() {
  try {
    ...
    form.value.requiredSkills = event.required_skills || ''

    // Format dates for datetime-local input
    if (event.start_time) {
      form.value.start_time = new Date(event.start_time).toISOString().slice(0, 16)
    }
    if (event.end_time) {
      form.value.end_time = new Date(event.end_time).toISOString().slice(0, 16)
    }

    // Set photo preview if exists
    if (event.photo) {
      photoPreview.value = event.photo
    }
...
async function handleSubmit() {
  try {
    ...
    error.value = err.response?.data?.message || 'Failed to update event. Please try again.'
  } finally {
